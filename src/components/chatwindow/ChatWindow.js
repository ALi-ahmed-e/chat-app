  ...
  return (
    <>
      <div className=' w-fsfsg h-[100vh] overflow-hidden'>
        ...
        <div className=' w-full h-[48px] bg-slate-500 flex items-center justify-start'>
        ...
        <div className='h-[87.1vh] w-full bg-slate-200 overflow-scroll'>

          {messages && messages.map(e => e.senderId == mydata.uid ? <div key={Math.random()} className=' min-w-16 w-20 rounded-lg rounded-tr-none mt-7 mr-3 bg-green-700 text-white'>
            {e.msg}</div> : <div key={Math.random()} className=' min-w-16 w-20 rounded-lg rounded-tr-none my-7 mr-3 bg-slate-600 text-white'>
            {e.msg}</div>)}
        ...
        <div className=' w-full h-[52px] bg-indigo-700 items-center flex justify-around'>
          ...
          <form onSubmit={(e) => sendmsg(e)} className='flex w-full justify-around items-center'>
            <button onClick={sendmsg} className=' bg-green-500 py-2 px-1 rounded-lg text-white hover:bg-green-600 h-10'>send <i className="fa-solid fa-paper-plane"></i></button>
